...
//		virtualmachine.loadAgent(jmxAgent, "com.sun.management.jmxremote");
		...
		Set<Object> keySet = agentProperties.keySet();

		for (Object object : keySet) {

			System.out.println("--------------------------------------------");
			System.out.println(object.toString());
			Object object2 = agentProperties.get(object);
		...
		OperatingSystemMXBean operatingSystemMXBean = ManagementFactory.newPlatformMXBeanProxy(connector.getMBeanServerConnection(), ManagementFactory.OPERATING_SYSTEM_MXBEAN_NAME,
			...
			if (method.isAccessible()) {
				if (method.getName().toString().startsWith("get") && method.getParameterTypes().length == 0) {
					...
					if (!memoryPoolMXBean.isCollectionUsageThresholdSupported()
							&& ("getCollectionUsageThreshold".equals(method.getName().toString()) || "getCollectionUsageThresholdCount".equals(method.getName().toString()))) {
						continue;
					}
