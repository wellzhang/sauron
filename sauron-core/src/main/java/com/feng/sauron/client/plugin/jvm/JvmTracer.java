...
public class JvmTracer {
	...
	private JvmTracer() {
		try {
			String jvm_switch = WatchableConfigClient.getInstance().get(SauronConfig.getAPP_NAME(), "jvm-switch", "ON");
			if ("ON".equals(jvm_switch)) {
	...
	private static class jvmMonitorInnerClass {
		private static final JvmTracer JVM_MONITOR = new JvmTracer();
	}
				...
				while (true) {
					try {
						...
						if ("ON".equals(jvm_switch)) {
							...
							try {
								String andWatch = WatchableConfigClient.getInstance().get(SauronConfig.getAPP_NAME(), "jvm-interval", "60"); // 停顿时间 单位 是s
