...
///**
// * 在集群模式下提供批量操作的功能。 <br/>
// * 由于集群模式存在节点的动态添加删除，且client不能实时感知（只有在执行命令时才可能知道集群发生变更）， 因此，该实现不保证一定成功，建议在批量操作之前调用 refreshCluster() 方法重新获取集群信息。<br />
...
//	}
//
//	private void innerSync(List<Object> formatted) throws JedisRedirectionException {
...
//		Jedis jedis = getJedis(JedisClusterCRC16.getSlot(new String(key)));
//
...
//		nodes.add(new HostAndPort("bredis1.fengjr.inc", 6379));
//		nodes.add(new HostAndPort("bredis2.fengjr.inc", 6381));
//		nodes.add(new HostAndPort("bredis3.fengjr.inc", 6383));
//
