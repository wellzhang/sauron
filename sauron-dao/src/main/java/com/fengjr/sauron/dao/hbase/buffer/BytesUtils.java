...
package com.fengjr.sauron.dao.hbase.buffer;
...
public final class BytesUtils {
    ...
    public static byte[] stringLongStringToBytes(final String string, final int maxStringSize, final long value1, final String value2) {
        if (string == null) {
            throw new NullPointerException("string must not be null");
        }
        if (maxStringSize < 0) {
        ...
        final byte[] seqs = toBytes(value2);
        if(seqs.length>SauronConstants.APPLICATION_SEQ_MAX_LEN)
            throw new IndexOutOfBoundsException("string seq is max " + seqs.length + ", string='" + value2 + "'");

        final byte[] buffer = new byte[LONG_BYTE_LENGTH + maxStringSize+ SauronConstants.APPLICATION_SEQ_MAX_LEN];
        writeBytes(buffer, 0, stringBytes);
        writeFirstLong0(value1, buffer, maxStringSize);
        writeBytes(buffer, maxStringSize+LONG_BYTE_LENGTH, seqs);
        return buffer;
    }

    public static byte[] stringLongToBytes(final String string, final int maxStringSize, final long value1) {
        if (string == null) {
            throw new NullPointerException("string must not be null");
        }
        if (maxStringSize < 0) {
        ...
        final byte[] buffer = new byte[LONG_BYTE_LENGTH + maxStringSize];
        writeBytes(buffer, 0, stringBytes);
        writeFirstLong0(value1, buffer, maxStringSize);
        return buffer;
    }
